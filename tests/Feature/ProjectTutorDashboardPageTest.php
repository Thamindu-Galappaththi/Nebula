...
class ProjectTutorDashboardPageTest extends TestCase
{
    ...
    public function test_pending_status_is_not_the_same_blue_as_review(): void
    {
        $this->actingAs($this->tutor)
            ...
            ->assertSee('id="projectPendingList"', false)
    ...
    public function test_pending_pagination_uses_ajax_partial_without_full_page(): void
    {
        ...
        $this->actingAs($this->tutor)
            ->get(route('project.tutor.dashboard'))
            ->assertOk()
            ->assertSee('PROJECT DASH PENDING 01')
            ->assertSee('PROJECT DASH PENDING 10')
            ->assertDontSee('PROJECT DASH PENDING 11')
            ->assertSee('Showing 1 to 10 of 11 pending requests');

        $pageTwo = $this->actingAs($this->tutor)
            ->withHeaders(['X-Requested-With' => 'XMLHttpRequest'])
            ->get(route('project.tutor.dashboard', ['pending_page' => 2]))
            ->assertOk()
            ->assertJsonStructure(['html']);

        $html = $pageTwo->json('html');
        $this->assertStringContainsString('PROJECT DASH PENDING 11', $html);
        $this->assertStringNotContainsString('PROJECT DASH PENDING 01', $html);
        $this->assertStringContainsString('Showing 11 to 11 of 11 pending requests', $html);
        $this->assertStringNotContainsString('Project Tutor Dashboard', $html);
        $this->assertStringNotContainsString('Recent Project Clearance Updates', $html);
    }
    ...
    private function seedPendingRequests(int $count): void
    {
        ...
        for ($i = 1; $i <= $count; $i++) {
            $student = Student::forceCreate([
                'title'              => 'Mr',
                'name_with_initials' => sprintf('Project Dash Pending %02d', $i),
                'full_name'          => sprintf('Project Dash Pending %02d Full', $i),
                'id_type'            => 'NIC',
                'id_value'           => sprintf('1993%06dV', $i),
                'gender'             => 'Male',
                'email'              => sprintf('project-dash-pending-%02d@test.lk', $i),
                'status'             => 'Registered',
            ...
            ClearanceRequest::forceCreate([
                'clearance_type' => ClearanceRequest::TYPE_PROJECT,
                'location'       => 'Welisara',
                'course_id'      => $course->course_id,
