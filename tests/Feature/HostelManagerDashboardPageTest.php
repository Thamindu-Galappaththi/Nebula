...
class HostelManagerDashboardPageTest extends TestCase
{
    ...
    public function test_pending_status_is_not_the_same_blue_as_review(): void
    {
        $this->actingAs($this->manager)
            ...
            ->assertSee('id="hostelPendingList"', false)
            ->assertSee('loadPendingPage', false)
            ->assertSee('hostel-pending-page', false);
    }
    ...
    public function test_pending_pagination_uses_ajax_partial_without_full_page(): void
    {
        ...
        $this->actingAs($this->manager)
            ->get(route('hostel.manager.dashboard'))
            ->assertOk()
            ->assertSee('HOSTEL DASH PENDING 01')
            ->assertSee('HOSTEL DASH PENDING 10')
            ->assertDontSee('HOSTEL DASH PENDING 11')
            ->assertSee('Showing 1 to 10 of 11 pending requests');

        $pageTwo = $this->actingAs($this->manager)
            ->withHeaders(['X-Requested-With' => 'XMLHttpRequest'])
            ->get(route('hostel.manager.dashboard', ['pending_page' => 2]))
            ->assertOk()
            ->assertJsonStructure(['html']);

        $html = $pageTwo->json('html');
        $this->assertStringContainsString('HOSTEL DASH PENDING 11', $html);
        $this->assertStringNotContainsString('HOSTEL DASH PENDING 01', $html);
        $this->assertStringContainsString('Showing 11 to 11 of 11 pending requests', $html);
        $this->assertStringNotContainsString('Hostel Manager Dashboard', $html);
        $this->assertStringNotContainsString('Recent Hostel Clearance Updates', $html);
    }
    ...
    private function seedPendingRequests(int $count): void
    {
        ...
        for ($i = 1; $i <= $count; $i++) {
            $student = Student::forceCreate([
                'title'              => 'Mr',
                'name_with_initials' => sprintf('Hostel Dash Pending %02d', $i),
                'full_name'          => sprintf('Hostel Dash Pending %02d Full', $i),
                'id_type'            => 'NIC',
                'id_value'           => sprintf('1992%06dV', $i),
                'gender'             => 'Male',
                'email'              => sprintf('hostel-dash-pending-%02d@test.lk', $i),
                'status'             => 'Registered',
