...
<div class="container-fluid">
    <div class="card shadow-sm p-3 mb-4 bg-white">
        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2">
            <div>
                <h3 class="fw-bold mb-1">Librarian Dashboard</h3>
                <small class="text-muted">Managing library clearance requests and reviewing student eligibility</small>
            </div>
            <a href="{{ route('library.clearance') }}" class="btn btn-primary btn-sm">
                Open library clearance
            </a>
        </div>
    </div>
    ...
    <div class="card shadow-sm p-4 mb-4 bg-white">
        ...
        <div id="librarianPendingList">
            @include('dashboards.partials.librarian_pending_list', ['pendingList' => $pendingList])
        </div>
    </div>

    <div class="card shadow-sm p-4 mb-4 bg-white">
        <h5 class="fw-semibold mb-1">Recent Library Clearance Updates</h5>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle text-center">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-muted py-3">No processed library updates yet</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
(function () {
    const list = document.getElementById('librarianPendingList');
    if (!list) {
        return;
    }
    ...
    function loadPendingPage(url, pushUrl) {
        ...
        .then(function (payload) {
            ...
            if (pushUrl) {
                history.pushState({ librarianPendingAjax: true }, '', url);
            }
        })
        .catch(function () {
    ...
    document.addEventListener('click', function (e) {
        const link = e.target.closest('#librarianPendingList a.librarian-pending-page');
        if (!link) {
            return;
        }
    ...
    window.addEventListener('popstate', function () {
        if (!document.getElementById('librarianPendingList')) {
            return;
        }
