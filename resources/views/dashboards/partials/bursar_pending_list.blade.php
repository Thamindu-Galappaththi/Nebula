    ...
    <table class="table table-hover align-middle text-center">
        ...
        <tbody>
            @forelse($pendingList as $req)
                @php
                    $reviewUrl = route('payment.clearance', array_filter([
                ...
                <tr>
                    ...
                    <td>{{ $req->requestedAtSriLanka()?->format('Y-m-d') ?? '-' }}</td>
                    <td>
...
<div class="d-flex justify-content-between align-items-center mt-3 pt-3 border-top">
    ...
    <div class="d-flex gap-2">
        <a href="{{ $pendingList->previousPageUrl() ?: '#' }}"
           class="btn btn-outline-secondary btn-sm bursar-pending-page {{ $pendingList->onFirstPage() ? 'disabled' : '' }}"
        ...
        <a href="{{ $pendingList->hasMorePages() ? $pendingList->nextPageUrl() : '#' }}"
           class="btn btn-outline-secondary btn-sm bursar-pending-page {{ $pendingList->hasMorePages() ? '' : 'disabled' }}"
