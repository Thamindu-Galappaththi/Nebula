...
<div class="container-fluid">
    ...
    <div class="card shadow-sm p-4 mb-4 bg-white">
        ...
        <div id="hostelPendingList">
            @include('dashboards.partials.hostel_pending_list', ['pendingList' => $pendingList])
        </div>
    </div>
...
(function () {
    const list = document.getElementById('hostelPendingList');
    if (!list) {
        return;
    }
    ...
    function loadPendingPage(url, pushUrl) {
        ...
        .then(function (payload) {
            ...
            if (pushUrl) {
                history.pushState({ hostelPendingAjax: true }, '', url);
            }
        })
        .catch(function () {
    ...
    document.addEventListener('click', function (e) {
        const link = e.target.closest('#hostelPendingList a.hostel-pending-page');
        if (!link) {
            return;
        }
    ...
    window.addEventListener('popstate', function () {
        if (!document.getElementById('hostelPendingList')) {
            return;
        }
